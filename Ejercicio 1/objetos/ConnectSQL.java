...
public class ConnectSQL {
    ...
    public void abrirConnec() {
        try {
            ...
            conexion = DriverManager.getConnection("jdbc:mysql://192.168.1.121:3306?useTimezone=true&serverTimezone=UTC",
                    "remote", "Jordi21_5");
 ...
 // Metodo para insertar datos en la tabla
    ...
    public void getValues(String db, String table_name) {
        try {
            ...
            int numeroBus = Integer.parseInt(JOptionPane.showInputDialog("Cuantas columnas quieres consultar"));
            String busqueda = "";
            String aux[]= new String[numeroBus];
            
            for (int i = 0; i < aux.length; i++) {
				
            	aux[i] = JOptionPane.showInputDialog("Columna " + (i+1));
            ...
            while (resultSet.next()) {
                ...
                for (int i = 0; i < aux.length; i++) {
					stringAux += aux[0] + ": " + resultSet.getString(aux[0] + "\n");
				}
    ...
    // Metodo para hacer coger el valor
    public void getValues(String db, String table_name, String busqueda) {
        try {
            String Querydb = "USE " + db + ";";
            Statement stdb = conexion.createStatement();
            stdb.executeUpdate(Querydb);
            ...
            while (resultSet.next()) {
                ...
                for (int i = 0; i < palabras.length; i++) {
                	aux += palabras[0] + ": " + resultSet.getString(palabras[0] + "\n");
				}
